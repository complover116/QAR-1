...
public class Player implements Serializable {
		...
		hitdelay --;
			...
			if(new Rectangle((int) x, (int)newY, 32,32).intersects(r)){
				...
				if(!this.onGround)
					...
					CurGame.lvl.TADs.add(new Particle(this.x+40-Math.random()*48,this.y+38-Math.random()*12,1, new Color(0,255,0), 10));
				}
				if(!this.onGround)
					for(int j = 0; j < 10; j ++)
						CurGame.lvl.TADs.add(new Particle(this.x+40-Math.random()*48,this.y+38-Math.random()*12,1, new Color(0,0,0), 10));
				this.onGround = true;
				this.y = r.getY() - 32;
	...
	public static Color getSkinColor(int skin) {
		if(skin == 1) return new Color(0,0,200);
		if(skin == 2) return new Color(200,0,0);
		if(skin == 3) return new Color(0,200,0);
		if(skin == 4) return new Color(200,200,0);
