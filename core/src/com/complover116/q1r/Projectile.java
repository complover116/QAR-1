	...
	public void tick(double deltaT) {
		...
		if (time > 0.02) {
			...
			for (int i = 0; i < 3; i++)
				if (Settings.ticktime < 0.25)
					GameWorld.ents.add(new Particle(x, y, 4, 4, this.color, (float) Math.random() * 200 - 100,
							(float) Math.random() * 200 - 100, false, true));
		...
		for (int i = 0; i < GameWorld.platforms.size(); i++) {
			if (new Rectangle(this.x, this.y, 16, 16).overlaps(GameWorld.platforms.get(i).bounds)) {
				for (int j = 0; j < 8; j++)
					GameWorld.ents.add(new Particle(x, y, 8, 8, this.color, (float) Math.random() * 160 - 80,
							(float) Math.random() * 160 - 80));
				this.isDead = true;
			}
		}
